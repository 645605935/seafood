...
class FangController extends AuthController {
    ...
    public function edit(){
        ...
        if(IS_POST){
            ...
            $type=A('Communal/Type');
            $row=M('Fang')->where('id='.$_GET['id'])->find();
    ...
    public function ajax_del(){
        ...
        if(is_array($ids_arr)){
            foreach ($ids_arr as $key => $value) {
                $fid=$value;
                $_fang_info=M('Fang')->where(array('id'=>$fid))->find();
                unlink('./Uploads'.$_fang_info['img']);

                if($res = $res = M('Fang')->where(array('id'=>$fid))->save(array('is_del'=>1))){
                    $data=array();
                    $data['code']=0;
                    $data['msg']='success';
