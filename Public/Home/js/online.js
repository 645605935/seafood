$(function(){
	var online_array=new Array(),color='info';
	online_array[0]='<li><a href="javascript:void(0);" class="btn btn-icon btn-'+color+' btn-squared" id="btn-meqia" onclick="_MEIQIA._SHOWPANEL()" target="_blank"><i class="fa fa-commenting" aria-hidden="true"></i></a></li>';
	//online_array[1]='<li><a href="http://crm2.qq.com/page/portalpage/wpa.php?uin=4000084433&aty=0&a=0&curl=&ty=1" class="btn btn-icon btn-'+color+' btn-squared" rel="nofollow" target="_blank"><i class="icon fa-qq" aria-hidden="true"></i></a></li>';
   online_array[1]='<li><button type="button" class="btn btn-icon btn-'+color+' btn-squared " data-toggle="modal" data-target="#indexFeed" id="feed-online"data-backdrop="false"><i class="icon fa-pencil" aria-hidden="true"></i></button></li>';
	online_array[2]='<li><button type="button" class="btn btn-icon btn-'+color+' btn-squared met-scroll-top" onclick="goTop()"><i class="icon wb-chevron-up" aria-hidden="true"></i></button></li>';
	online_array.unshift("<div class='met-online'><ul class='blocks hidebacktop'>");
	online_array.push("</ul></div>");
	$('body').append(online_array.join(''));
...
$(function(){
    ...
    if($("a.user_name").length){
        ...
        $.ajax({
            url:'http://app.metinfo.cn/index.php?lang=cn&n=platform&c=meiqia&a=doinfo',
            type:'GET',
            dataType:'jsonp',
            data:{code:meqia_code},
...
function pyRegisterCvt(_orderno){
    ...
    if (w.parent!=w){f=b;b=c;c=f;};u='//stats.ipinyou.com/cvt?a='+e('5Kh.1kh.trevdniVmRo5ujRXGrzrzP')+'&c='+e(h?h[2]:'')+'&s='+e(i?i[2].match(/jump\%3D(\d+)/)[1]:'')+'&u='+e(b)+'&r='+e(c)+'&rd='+(new Date()).getTime()+'&OrderNo='+e(_orderno)+'&e=';
    (new Image()).src=u;
}
/*获取当前时间*/
function nowTime(){
